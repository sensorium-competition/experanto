...
def test_nearest_neighbor_interpolation(duration, fps, image_frame_count, num_videos, encoded):
    with create_screen_data(
        ...
        assert np.all(expected_frames >= 0) and np.all(expected_frames <= 1), \
            f"Expected values outside [0, 1] range: [{expected_frames.min():.3f}, {expected_frames.max():.3f}]"
        assert np.all(interp >= 0) and np.all(interp <= 1), \
        ...
        if encoded:
            ...
            min_correlation = 0.95  # Expect high correlation despite compression artifacts
