...
@pytest.mark.parametrize("device_ranges, expected_start, expected_end", DEVICE_TIME_RANGE_CASES)
def test_experiment_start_end_time_reflects_union(tmp_path, device_ranges, expected_start, expected_end, n_signals):
    """
    ...
    with ExitStack() as stack:
        for name, (start, end) in zip(device_names, device_ranges):
            stack.enter_context(make_sequence_device(tmp_path, name, start=start, end=end, n_signals=n_signals))
