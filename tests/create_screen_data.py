...
def create_screen_data(
    duration=10.0, frame_shape=(32, 32), fps=10.0, image_frame_count=10, num_videos=1, encoded=False
):
    try:
        ...
        for i in range(min(image_frame_count, n_frames)):
            if encoded:
                # Save as JPEG
                img = Image.fromarray(frames[i], mode='L' if len(frame_shape) == 2 else 'RGB')
        ...
        if start_vid < n_frames:
            ...
            for vid_idx in range(num_videos):
                ...
                if encoded:
                    ...
                    # Save as numpy array (original behavior)
                    np.save(data_dir / f"{file_idx:05d}.npy", video_array.astype(np.float32))
                    file_ext = ".npy"
