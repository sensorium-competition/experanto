...
class TimeInterval(typing.NamedTuple):
    ...
    def find_intersection_between_two_intervals(self, other_interval: "TimeInterval") -> "TimeInterval":
...
def find_intersection_between_two_interval_arrays(interval_array_1: List[TimeInterval], interval_array_2: List[TimeInterval]) -> List[TimeInterval]:
...
def find_intersection_across_arrays_of_intervals(intervals_array: List[List[TimeInterval]]) -> TimeInterval:
    
    common_interval_array = intervals_array[0]

    for interval_array in intervals_array[1:]:
        common_interval_array = find_intersection_between_two_interval_arrays(common_interval_array, interval_array)
    
    return uniquefy_interval_array(common_interval_array)

def find_union_across_arrays_of_intervals(intervals_array: List[List[TimeInterval]]) -> TimeInterval:
    union_array = []
    for interval_array in intervals_array:
        union_array.extend(interval_array)
    return uniquefy_interval_array(union_array)

def find_complement_of_interval_array(start: float, end: float, interval_array: List[TimeInterval]) -> List[TimeInterval]:
    """
...
def get_stats_for_valid_interval(intervals: List[TimeInterval], start_time: float, end_time: float) -> str:
    """
    ...
    for interval in unique_intervals:
        ...
        if clamped_end > clamped_start: # Only consider intervals with positive duration within the range
    ...
    if valid_intervals_in_range:
        valid_durations = np.array([inv.end - inv.start for inv in valid_intervals_in_range])
    ...
    invalid_intervals = find_complement_of_interval_array(start_time, end_time, valid_intervals_in_range)
